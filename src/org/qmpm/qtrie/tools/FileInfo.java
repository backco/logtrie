...
public interface FileInfo<T extends Collection<? extends List<?>>> {
	...
	void append(FileInfo<T> mainFile);

	File getFile();

	Collection<? extends List<?>> getLoadedFile();

	void setLoadedFile(Collection<? extends List<?>> loadedFile) throws Exception;

	boolean fileLoaded();

	void cutDownFile(int i, int j);
	...
	void saveAs(String path) throws FileNotFoundException, IOException;
